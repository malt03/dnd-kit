...
function useDroppableMeasuring(containers, {
  ...
  const measureDroppableContainers = React.useCallback((ids = []) => {
...
function useObservedRect(element, measure = getClientRect) {
  const initialRect = React.useMemo(() => element ? measure(element) : null, [element, measure]);
  const [rect, measureRect] = React.useReducer(reducer, null);
  const handleMutations = React.useCallback(records => {
    if (!element) {
      return;
    }
